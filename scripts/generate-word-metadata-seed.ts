 ...
 * Execute in Supabase after setup_gift_v0.sql and words seed.
 */
...
console.log('INSERT INTO public.word_metadata (word_id, character, pinyin, en_meaning, video_storage_path, video_storage_bucket)')
console.log('VALUES')
console.log(
  rows
    .map(
      (r) =>
        `  (${sqlLiteral(r.word_id)}, ${sqlLiteral(r.character)}, ${sqlLiteral(r.pinyin)}, ${sqlLiteral(r.en_meaning)}, ${sqlLiteral(r.video_storage_path)}, ${r.video_storage_bucket ? sqlLiteral(r.video_storage_bucket) : 'NULL'})`,
    )
    .join(',\n') +
